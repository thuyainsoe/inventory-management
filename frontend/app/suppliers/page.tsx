...
const getPaymentTermsBadgeColor = (terms: string) => {
  ...
  if (terms.includes('15') || terms.includes('20')) return 'bg-blue-100 text-blue-800'
...
export default function SuppliersPage() {
  ...
  const columns: ColumnDef<Supplier>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Badge className={getPaymentTermsBadgeColor(row.getValue('paymentTerms'))}>
  ...
  const activeSuppliers = suppliers.filter(s => {
    const daysSinceUpdate = Math.floor((new Date().getTime() - new Date(s.updatedAt).getTime()) / (1000 * 60 * 60 * 24))
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Comprehensive list of all suppliers with ratings, contact information, and payment terms
            </CardDescription>
          </CardHeader>
          <CardContent>
            <DataTable
