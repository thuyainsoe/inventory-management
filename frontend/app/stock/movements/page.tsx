...
import { Plus, TrendingUp, TrendingDown, RefreshCw, Truck, Package, ArrowUpCircle, ArrowDownCircle } from 'lucide-react'
  ...
  const columns: ColumnDef<ExtendedStockMovement>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div>
          ...
          <div className="text-sm text-gray-500 font-mono">{row.original.productSku}</div>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="max-w-xs">
          <div className="text-sm text-gray-600 truncate" title={row.getValue('notes')}>
            {row.getValue('notes')}
          </div>
        </div>
      ),
    ...
    createActionColumn<ExtendedStockMovement>(
      ...
      [
        ...
        {
          label: 'Print Receipt',
          onClick: (movement) => console.log('Print receipt for:', movement.reference),
        },
      ]
    ),
  ]
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <div className={`h-2 w-2 rounded-full ${netChange >= 0 ? 'bg-green-500' : 'bg-red-500'}`} />
            </CardHeader>
            <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Complete history of all stock movements including receipts, shipments, adjustments, and transfers
            </CardDescription>
          </CardHeader>
          <CardContent>
            <DataTable
              ...
              onRowClick={(movement) => console.log('Movement clicked:', movement)}
            />
          </CardContent>
        </Card>
      </div>
